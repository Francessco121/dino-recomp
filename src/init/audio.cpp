...
void dino::init::queue_samples(int16_t* audio_data, size_t sample_count) {
    ...
    float cur_main_volume = dino::sound::get_main_volume() / 100.0f; // Get the current main volume, normalized to 0.0-1.0.
...
size_t dino::init::get_frames_remaining() {
...
void dino::init::set_frequency(uint32_t freq) {
    sample_rate = freq;
    
    update_audio_converter();
}

void dino::init::reset_audio(uint32_t output_freq) {
