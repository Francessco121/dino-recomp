...
extern void *memcpy_recomp(void *dst, const void *src, size_t size);
extern ldiv_t ldiv_recomp(long num, long denom);
...
RECOMP_PATCH void _Ldtob(_Pft* px, char code) {
    char buff[BUFF_LEN];
    char *p;
    double ldval;
    short err;
    ...
    if (err > 0) {
        memcpy_recomp(px->s, err == 2 ? "NaN" : "Inf", px->n1 = 3);
        return;
    } else if (err == 0) {
        nsig = 0;
        xexp = 0;
...
void _Genld(_Pft* px, char code, unsigned char* p, short nsig, short xexp) {
    ...
    if (code == 'f' || (code == 'g' || code == 'G') && xexp >= -4 && xexp < px->prec) {
        ...
        if (xexp <= 0) {
            ...
            memcpy_recomp(&px->s[px->n1], p, px->n2 = nsig);
            px->nz2 = px->prec - nsig;
        } else if (nsig < xexp) {
            memcpy_recomp(&px->s[px->n1], p, nsig);
            px->n1 += nsig;
            px->nz1 = xexp - nsig;
            if (px->prec > 0 || (px->flags & 8)) {
                px->s[px->n1] = point;
            ...
            memcpy_recomp(&px->s[px->n1], p, xexp);
            px->n1 += xexp;
            nsig -= xexp;
            ...
            memcpy_recomp(&px->s[px->n1], &p[xexp], nsig);
            px->n1 += nsig;
            px->nz1 = px->prec - nsig;
        }
    } else {
